...
## Рабочее правило: локальная разработка и проверка

Код и документацию редактируем локально, но **не запускаем локальные сборки**
(`npm run build`, `nuxi build`, `yarn build`, `composer install` и аналогичные
команды). После правок нужно закоммитить и запушить изменения, затем выполнить
pull, необходимые сборки/перезапуски и функциональную проверку **на сервере**
по этому runbook. Это правило действует и для небольших изменений фронтенда.
