...
- **2026-09-04 (4)** — «Город отправки» в настройках СДЭК
  ...
  - backend: `GET /api/third-party-integrations/cdek/warehouses?query&limit`
    (admin, sanctum) — справочник точек СДЭК, принимающих отправления, одним
    запросом `/v2/deliverypoints?is_reception=true`, кэш на сутки
    (`cdek:sender-offices:ru`); поиск по
