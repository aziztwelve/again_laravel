...
- **2026-09-05 (2)** — выравнивание кнопки «Отменить оплату» на странице
  ...
  стояла в правой половине грида (`col-span-2 pt-8` после поля «Дата
  оплаты»). Теперь `lg:col-start-1` (вместо `pt-8`) переносит её на новую
  строку в левую колонку — левый край совпадает с полем «Дата оплаты», под
  строкой «ID транзакции». Правка только в классах
  `OrderStatuses.vue` (vue-admin). Backend не менялся, миграций/сидов нет;
  vue-admin пересобран на сервере, laravel/nuxt не трогались. Проверено
  headless-браузером с временным админ-токеном (удалён): desktop — delta left
  «кнопка → поле» = 0 (обе x=344, кнопка под полем с межстрочным отступом
  грида), mobile 375px — delta 0, JS-ошибок нет. Скриншоты:
  `/tmp/opencode/pwtest/order-before.png`, `order-after.png`. Smoke: pm2
  online, /up → 200. Head: vue-admin `246f741`.
