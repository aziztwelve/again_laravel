...
- **2026-09-05 (2)** — выравнивание кнопки «Отменить оплату» на странице
  заказа (https://againdev3.ru/admin/order/71716, блок «Статусы»): кнопка
  стоит в клетке справа от «Дата оплаты» (колонки 3-4 на lg), но уходила на
  4px ниже инпута (pt-8 = 32px при фактических label 24px + mt-1 4px).
  Уточнение по просьбе пользователя: кнопка должна быть **справа от поля**
  и по верху совпадать с инпутом даты. Заменено `pt-8` → `pt-7` (28px,
  = label + mt-1). `OrderStatuses.vue` (vue-admin), правка только в
  классах. Backend не менялся, миграций/сидов нет; vue-admin пересобран на
  сервере, laravel/nuxt не трогались. Проверено headless-браузером с
  временным админ-токеном (удалён): desktop — кнопка y=260 = инпут y=260
  (высота 36=36), x=740 (инпут 344+380, отступ 16px грида); mobile 375px —
  кнопка под полем во всю ширину, по левому краю; JS-ошибок нет. Smoke:
  pm2 online, /up → 200. Head: vue-admin `c832d1c`.
