...
## Актуальная логика промокодов (2026-07-31)

`allow_promo_codes` не заменяет подарок скидкой. Подарок обязателен для каждой
применённой акции. Промокод разрешён вместе с набором подарков, если хотя бы у
одной акции из набора включён `allow_promo_codes`; если флаг выключен у всех —
промокод отклоняется. `use_discount_instead = true` больше не допускается.

После промокода витрина пересчитывает применимость акций по новой сумме:
подарок, который выпал ниже порога, остаётся показанным как недоступный и не
передаётся в заказ.
