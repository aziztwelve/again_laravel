...
После `Pay` backend публикует `OrderPaid`; зарегистрированный listener ставит
`CreateYandexOrderJob` в очередь для заказов с `yandex_courier` или
`yandex_pickup`. Повторный Pay не создаёт вторую заявку: задача проверяет
существующий `YandexOrder` и `claim_id`.
