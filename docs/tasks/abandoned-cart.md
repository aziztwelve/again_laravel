...
аналитика работают только для **авторизованных клиентов**. Гостевые корзины
исключены из сценария брошенных корзин: не помечаются `abandoned`, не попадают в
аналитику/список и не получают напоминания.
**Дата:** 2026-06-24 (обновлено 2026-06-26)
**Раздел админки:** Аналитика → Брошенные корзины

## Актуализация жизненного цикла (2026-07-31)

Эта актуализация имеет приоритет над всеми предыдущими интервалами и правилами
статуса в документе.

- Через **30 минут** без активности корзина входит в цепочку напоминаний, но
  остаётся со статусом `active`. Момент фиксируется в `abandoned_at` как начало
  цепочки.
- Первое касание отправляется через **2 часа от последней активности** — то есть
  через 1 час 30 минут после начала цепочки. Второе и третье касания — через
  **24** и **48 часов** от последней активности соответственно.
- Ограничения по времени суток нет: касания отправляются сразу при наступлении
  срока, независимо от часового пояса клиента.
- Обработчик запускается каждую минуту, поэтому задержка определяется только
  очередью уведомлений, а не ожиданием следующего почасового запуска.
- Статус меняется на `abandoned` только после постановки в отправку третьего
  касания, если к этому моменту корзина не была оформлена.
- Если заказ оформлен после любого касания, корзина сразу получает статус
  `ordered`; дальнейшие касания не отправляются.

## Актуализация сценария писем (2026-07-27)

Цепочка разделяется по типу клиента, который определяется **на момент
отправки**: если у клиента есть хотя бы один оформленный заказ — это повторный
клиент, иначе — новый. Гостевые корзины по-прежнему исключены.

Время считается от последней активности в корзине: первое касание через
...
| Шаг | Когда | Тема | Содержание |
|---|---:|---|---|
| 1 | 2 ч | `Ваш выбор ждёт ✨ (и мы поможем с размером)` | Обращение по имени; сообщить, что выбор сохранён; предложить ответить на письмо для помощи с размером, тканью или посадкой; кнопка «Вернуться в корзину»; напомнить о бесплатной доставке для первого заказа. |
| 2 | 24 ч | `Дарим 10% на первый заказ 🤍` | Обращение по имени; скидка 10% на корзину, промокод `FIRST10` на 48 часов; преимущества: точные лекала и дышащие ткани, обмен размера при соблюдении условий, дискретная упаковка и отправка в день заказа; кнопка «Оформить заказ со скидкой». |
| 3 | 48 ч | `Осталось 6 часов: корзина освободится ⏳` | Сообщить об окончании резерва через 6 часов; акцент на размере и цвете, бесплатной доставке и подарке к первому заказу; кнопка «Завершить заказ сейчас»; мягкая возможность проигнорировать письмо. |

### 🔵 Воронка 2 — повторный клиент (уже покупал)

Цель: использовать доверие, сократить путь к заказу и подчеркнуть преимущества
постоянного клиента.

| Шаг | Когда | Тема | Содержание |
|---|---:|---|---|
| 1 | 2 ч | `[Имя], вы снова выбрали нас ❤️` | Поблагодарить за возвращение, напомнить о корзине, предложить помощь с сочетанием прошлых покупок/размером; кнопка «Вернуться в корзину»; упомянуть приоритетную обработку. |
| 2 | 24 ч | `Бесплатная доставка для вас 📦 (уже в корзине)` | Сообщить, что бесплатная доставка применяется автоматически; преимущества: быстрая отправка, обмен при соблюдении условий, популярные модели быстро разбирают; кнопка «Оформить заказ в 1 клик». |
| 3 | 48 ч | `Последний шанс: ваш заказ уходит с резерва 🕊️` | Сообщить, что через несколько часов резерв снимется; подчеркнуть риск потерять размер/ждать поставку; кнопка «Завершить заказ сейчас» и возможность написать в ответ. |

### Технические правила

- Содержимое должно быть одинаково понятным в email и мессенджерах: email
  получает HTML-версию с кнопкой, мессенджер — текст и ссылку.
- Имя берётся из профиля клиента; если его нет, используется нейтральное
  обращение без подстановки имени.
- Шаг 2 для нового клиента использует только реально существующий и активный
  промокод `FIRST10`; при его отсутствии отправка не должна обещать скидку.
- Ручная отправка из админки использует первый сценарий соответствующей воронки
  и не выдаёт промокод.

> ⚠️ **Границы фичи.** `universal-cart.md` сохраняет серверную корзину для
> гостей через `guest_token`, но abandoned-cart сознательно работает только с
> корзинами, у которых есть `client_id`. Гостевые корзины обслуживаются как
> обычная активная/оформленная корзина и не участвуют в триггерной цепочке.
...
Это наполняет «Заказы»/«Конверсию» и останавливает цепочку для клиентских
корзин. Гостевые корзины не участвуют в abandoned-cart аналитике и цепочке.
...
- **Логика:** переиспользуем `AbandonedCartService` — выделить публичный метод
  ...
  - проверить, что корзина принадлежит клиенту (`client_id IS NOT NULL`), не
    `ordered` и не пуста;
...
- **Идемпотентность:** ручная отправка **не** ограничена UNIQUE-шагами (менеджер
  ...
  минут на корзину) и проверкой наличия контакта.
...
- **Определение:** `versions_count` = число строк `cart` с той же идентичностью
  ...
  - гостевые корзины в этом разделе не показываются.
...
- **Гостевые корзины** (`client_id IS NULL`) — не участвуют в abandoned-cart:
  не помечаются `abandoned`, не входят в аналитику/список, по ним не отправляются
  триггерные или ручные напоминания.
...
5. ✅ **Гости — решено:** abandoned-cart работает только для авторизованных
   клиентов (есть `client_id` и контакт). Гостевые корзины не входят в аналитику
   и не участвуют в цепочке, даже если у корзины есть email/phone/consent.
...
- `OrderCreationService::createOrder` → `linkCartToOrder($order, $clientId)`:
  ...
  оформления автоматически для клиентских корзин.
...
- **Шаг G — Счётчик версий корзины:** агрегат `versions_count` в
  `CartController@carts` по `client_id` + подпись «N версий» под ID корзины во
  фронте.
