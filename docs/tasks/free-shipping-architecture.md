...
```mermaid
flowchart LR
    E["Менеджер правит заказ<br/>PUT /api/orders/{order}"] --> W{"менялось что-то<br/>влияющее на условия?<br/>items · promo_code ·<br/>способ доставки/оплаты ·<br/>delivery_cost"}
    W -->|нет| SKIP["стоимость доставки не трогаем<br/>(комментарий, статус, адрес)"]
    W -->|да| RE["applyToOrder заново"]
    RE --> R1["сумма выкупа упала ниже порога<br/>→ доставка снова платная"]
    RE --> R2["сумма выросла до порога<br/>→ доставка становится 0"]
    RE --> R3["менеджер вписал цену руками<br/>→ она становится тарифной базой<br/>(вернётся при отмене правила)"]
```
