...
2. Порог: `qualifying_amount >= min_order_amount` → бесплатно; на копейку ниже —
   платно.
3. Скидки: промокод и «скидка вместо подарка» опускают сумму ниже порога →
   доставка платная (ключевое требование заказчика).
4. Товарное условие: считается сумма только выбранных товаров; подарки
   (`is_gift`) не считаются.
5. Несколько правил: выбирается правило с меньшим порогом, `free_shipping_rule_id`
   записан, `delivery_cost_original` сохранён.
6. Интеграция чекаута: `POST /api/public/orders` → `delivery_cost = 0`,
   `total_amount` пересчитан.
7. Админ CRUD: создание с пивотами, обновление через `sync`, валидация кодов.

## 11. Этапы

1. Спека (этот документ).
2. Миграции + модель + связи.
3. `FreeShippingService` (+ контекст, матч, прогресс).
4. Admin CRUD API + `options` + публичный `evaluate`.
5. Применение в чекауте (публичный + админский) + `FreeShippingRuleSeeder`.
6. Тесты backend.
7. Админка: меню, список, форма.
8. Витрина: «Бесплатно», строка «Доставка», прогресс-бар.
9. Деплой и проверка на сервере по [`../deploy-runbook.md`](../deploy-runbook.md):
   push трёх репозиториев → pull → `composer install` + `migrate` →
   `php artisan db:seed --class=FreeShippingRuleSeeder --force` (идемпотентный) →
   сборки `nuxt-shop`/`vue-admin` → smoke.

## 12. Открытые вопросы

1. **Товарное условие** — принято Решение 2 (порог по выбранным товарам).
   Если нужна альтернатива (товары = фильтр, порог по всей корзине) — сказать
   до реализации этапа 3.
2. **Подарочная карта** — сейчас не уменьшает сумму выкупа (Решение 3).
3. **Легаси-способы доставки** (Boxberry, Почта России) в мультивыборе служб не
   участвуют: на витрине активны только СДЭК и Яндекс. При появлении новых
   служб набор расширяется в `config/free_shipping.php`.
