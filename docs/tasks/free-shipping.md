...
4. **Ручная стоимость доставки в админском заказе** — ждём подтверждения.
   Сейчас правила сильнее: если менеджер вписал 800 ₽, а условия правила
   выполнены, в заказе будет 0 ₽ (введённая сумма сохраняется как тарифная база
   и вернётся, когда правило перестанет подходить). Если ручная цена должна
   переживать автоматику — нужен флаг `delivery_cost_manual`, отключающий
   пересчёт для конкретного заказа.

   **Закрыто 2026-08-18:** пересчёт при изменении состава заказа — включён
   (решение заказчика), см. §7.
