...
товарного условия (§12.1). Вопросы по пересчёту при редактировании заказа и по
приоритету ручной цены доставки — закрыты (§7, §12.4).
**Дата:** 2026-08-18
...
**Редактирование заказа в админке** (`OrderUpdateService::update()` →
`recalculateFreeShipping()`): правила переприменяются, если менялось что-то
влияющее на условия — `items`, `promo_code`, `delivery_method_id`,
`payment_method`, `delivery_cost`, `delivery_data`, `gift_card_amount`. Убрали
товары и сумма выкупа упала ниже порога — доставка снова платная; добавили —
становится бесплатной. Правка комментария, статуса или адреса стоимость
доставки не трогает. Ошибка пересчёта пишется в лог и не ломает сохранение
заказа.

**Ручная стоимость доставки.** Если менеджер сам изменил `delivery_cost`, это
значение становится новой тарифной базой: при сработавшем правиле в заказе будет
0, но в `delivery_cost_original` ляжет введённая сумма — и вернётся именно она,
когда правило перестанет подходить. Приоритета над правилами ручная цена **не
имеет** — решение подтверждено заказчиком 2026-08-18 (§12.4), флаг
`delivery_cost_manual` не вводится.
...
`tests/Feature/Delivery/FreeShippingTest.php` — **25 тестов, 86 проверок, зелёные**
...
7. Прогресс: остаток до порога; при взятом нижнем пороге подсказка показывает
   следующий тариф.
8. Редактирование заказа в админке: убрали товары → доставка снова платная;
   добавили → бесплатная; ручная стоимость доставки становится новой тарифной
   базой и возвращается при отмене правила; правка не влияющих полей
   (комментарий) стоимость не трогает.
9. Админ CRUD: создание с пивотами (товары/страны/регионы), обновление через
...
4. **Ручная стоимость доставки в админском заказе — решено 2026-08-18:
   правила сильнее, оставляем как есть.** Если менеджер вписал 800 ₽, а условия
   правила выполнены, в заказе будет 0 ₽; введённая сумма сохраняется в
   `delivery_cost_original` и вернётся, когда правило перестанет подходить.
   Чтобы взять деньги за доставку при сработавшем правиле, менеджер правит или
   выключает само правило. Флаг `delivery_cost_manual` не вводим.

   **Также закрыто 2026-08-18:** пересчёт при изменении состава заказа —
   включён (решение заказчика), см. §7.
...
- **2026-08-18** — по решению заказчика включён пересчёт правил при
  редактировании заказа в админке (`OrderUpdateService::recalculateFreeShipping`).
  Тесты 25/25. Проверено на живой БД в откаченной транзакции: 5040 ₽ → доставка 0;
  менеджер убрал товары (2520 ₽) → доставка 390, правило снято, итог 2910;
  вернул товары (6300 ₽) → доставка 0, правило снова применено; правка
  комментария стоимость не изменила. Регресс-прогоны AbandonedCartTest (31),
  UniversalCartTest (16), OrderNotificationServiceTest (7),
  PromotionStackingTest (7) — зелёные.
- **2026-08-18** — заказчик подтвердил: приоритета над правилами ручная
  стоимость доставки не получает (§12.4). Изменений в коде не потребовалось.
