...
> Актуально с 2026-07-31: подарок и промокод суммируются, а не выбираются как
> альтернативы. Поле промокода доступно, если среди применённых акций есть хотя
> бы одна с `allow_promo_codes = true`. Если флаг выключен у всех — промокод
> отклоняется. Подарок обязателен, `use_discount_instead = true` не допускается.

Система акций — механизм автоматического применения спецпредложений к заказу. Когда корзина пользователя соответствует условиям акции (наличие товара-триггера, минимальная сумма), ему предлагается бесплатный подарок; промокод при разрешённом флаге применяется дополнительно.
...
- [ ] `allow_promo_codes = true` хотя бы у одной акции → промокод применяется вместе с подарками
