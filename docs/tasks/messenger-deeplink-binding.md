...
**Статус:** В реализации — бэкенд и витрина готовы, требуется прогон миграции и
смоук-тест на dev-стенде (Docker/MySQL)
**Дата:** 2026-07-12
...
Осталось (требует dev-стенда с БД):
- Прогнать миграцию `php artisan migrate` (в этом окружении Docker/MySQL не
  поднят — миграция не выполнялась).
- Заполнить `max_user_id` VK/MAX — проверить, что мессенджеры реально
  присылают `start`/`ref` в первом апдейте (особенно VK `ref`).
