...
class YandexDeliveryClient
{
    ...
    public function request(string $httpMethod, string $method, array $payload = [], ?int $orderId = null, ?string $claimId = null, array $query = []): array
    {
        $baseUrl = rtrim((string) ($this->settings['base_url'] ?? ''), '/');
        if (!$this->settings['enabled'] || !$this->settings['token'] || !$baseUrl) {
            return $this->result(503, ['message' => 'Интеграция Яндекс.Доставки не настроена.'], $httpMethod, $method, '', $payload, $orderId, $claimId, 0);
        }
        ...
        try {
            ...
            if (!empty($this->settings['client_id'])) {
                $request = $request->withHeaders(['X-B2B-Client-Id' => $this->settings['client_id']]);
            }
            ...
            return $this->result($response->status(), $data, $httpMethod, $method, $url, $payload, $orderId, $claimId, $duration, $response->body());
        } catch (ConnectionException $exception) {
            $duration = (int) ((hrtime(true) - $started) / 1_000_000);
            Log::warning('Yandex Delivery API connection failed', ['method' => $method, 'error' => $exception->getMessage()]);
            return $this->result(503, ['message' => 'Не удалось подключиться к API Яндекс.Доставки.'], $httpMethod, $method, $url, $payload, $orderId, $claimId, $duration);
        }
    }

    private function result(int $status, array $data, string $httpMethod, string $method, string $url, array $requestBody, ?int $orderId, ?string $claimId, int $duration, ?string $body = null): array
    {
        $successful = $status >= 200 && $status < 300;
        YandexApiLog::create([
            'order_id' => $orderId, 'claim_id' => $claimId, 'direction' => 'request',
